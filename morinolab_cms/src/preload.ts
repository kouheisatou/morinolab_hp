...
contextBridge.exposeInMainWorld('api', {
  ...
  getTableData: (type: string): Promise<{ header: string[]; rows: Record<string, string>[] }> => ipcRenderer.invoke('get-table-data', type),
  updateCell: (type: string, id: string, column: string, value: string): Promise<void> => ipcRenderer.invoke('update-cell', type, id, column, value),
  selectThumbnail: (type: string, id: string): Promise<string | null> => ipcRenderer.invoke('select-thumbnail', type, id),
  resolvePath: (type: string, rel: string): Promise<string> => ipcRenderer.invoke('resolve-path', type, rel),
});
